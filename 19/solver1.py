...
#          print(f'{f} - {to}: {k} -> {m}')
          ...
          doingn[-1][2] = r
          #assert len(todo)+len(doingn)+len(doing)+len(done) == len(scanner),f'{len(todo)} {len(doingn)} {len(doing)} {len(done)} {[x[0] for x in todo]} {[x[0] for x in doingn]} {[x[0] for x in doing]} {[x[0] for x in done]}'
          break
...
for s in done:
  r = s[2]
  p = s[1]
  for row in range(scanner[s[0]].shape[0]):
    beacon = scanner[s[0]][row]
    t_pos = np.matmul(beacon, r) + p
    v = str(int(t_pos[0])) + ',' + str(int(t_pos[1])) + ',' + str(int(t_pos[2]))
    if v in d:
      d[v] += 1
    else:
